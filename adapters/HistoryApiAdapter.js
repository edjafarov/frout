function HistoryApiAdapterFactory(mount, rootElement, context){
  var adapter = {
    //renderData is a hash of data, params, and component
    // per resolved part of url
    renderer: function(renderData){
      var renderArr = Object.keys(renderData).sort().map(function(mask){
        return {
          mask: mask,
      ...
      function renderComp(renderArr){
        ...
        delete partial.params.children;
  ...
  return function HistoryApiAdapter(PPRouter){

    function handlePop(event){
      mount(adapter.renderer(PPRouter.prepareRenderData(event.state)));
    }
    ...
    rootElement.onclick = function(e){
      ...
      if(a && a.nodeType == 1
        ...
        if(document.location.pathname !== a.pathname) {
          PPRouter.handleURL(a.pathname, context)
            .then(function(data){
              //memo renderData
              data.renderData = data.renderData || {};
              var renderDataKeys = Object.keys(data.renderData);
              renderDataKeys.forEach(function(key){
                memo[key] = data.renderData[key];
              })
              var resolvedModels = Object.keys(data.handler.resolvedModels);
              resolvedModels.forEach(function(key){
                if(!~renderDataKeys.indexOf(key) && memo[key]) data.renderData[key] = memo[key];
              },[])

              //stringify renderData and push it in history
              //console.log(data)
...
module.exports = HistoryApiAdapterFactory;
