...
function chn(first, second){
  return function(){
    return Promise.resolve().then(first).then(second);
  }
}
...
describe('Router should use adapter', function(){
  ...
  PPRouter(function(PPRouter){
    PPRouter('/posts', function(PPRouter){
      PPRouter('/:id').do(chn(postsId, renderMock("MockID")))
    }).do(chn(rootPosts,renderMock("MockPosts")))
  }).do(chn(root, renderMock("MockRoot")))
...
describe('Router should use adapter and work with components', function(){
  ...
  PPRouter(function(PPRouter){
    PPRouter('posts', function(PPRouter){
      PPRouter(':id').component(componentID).do(chn(postsId,renderMock("MockID")))
    }).component(componentPosts).do(chn(rootPosts,renderMock("MockPosts")))
  }).component(componentRoot).do(chn(root,renderMock("MockRoot")))
